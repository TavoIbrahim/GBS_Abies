...
# These command lines were used to perform ADMIXTURE analyses

# 1.- We created .ped and .map files using PLINK v1.90b6.11 64-bit
./plink --vcf ../out_TMVB/ParFiltered_1SNPpop_mac2_mmiss0.50.recode.vcf --recode12 --allow-extra-chr --out ../out_TMVB/admixture_TMVB/ParFiltered_1SNPpop_mac2_mmiss0.50
# Because ADMIXTURE outputs will always be put in the current working directory (bin), we moved to ../out_TMVB/admixture_TMVB/ 
cd ../out_TMVB/admixture_TMVB/

# 2.- Run ADMIXTURE Version 1.3.0
prefix=ParFiltered_1SNPpop_mac2_mmiss0.50
for K in {1..8}; do
	for r in {1..10}; do
		./admixture -s ${RANDOM} --cv ParFiltered_1SNPpop_mac2_mmiss0.50.ped $K | tee log.$prefix.K${K}r${r}.out
