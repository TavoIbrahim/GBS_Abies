# This script was created by Alicia Mastretta-Yanes (https://github.com/AliciaMstt/RAD-error-rates) and slightly modified by Gustavo Ibrahim Giles-Pérez (https://github.com/TavoIbrahim).

LociAllele_error <- function(liSNPs, parametro){ 
  ...
  ##### Variables:
  # liSNPs =  a genlight object
  # parametro = character string to label the stacks parameters or data used 
...
#  ) 
  ...
  repliDiff<-matrix(ncol=11)
  colnames(repliDiff)<-c("pair", "nloci", "nMissLoc", "MissTotProp", "shareMissLoc", "loci.mismatches", "unshareMissLoc", "loci.error.rate", "n.loci.woNA", "allele.mismatches", "allele.error.rate")
  repliDiff<-repliDiff[-1,]
  
  for(n in 1:npairs) {
    
    srpair<-match(pairs[n,], samples) # Toma cada uno de los pares
    srpair <- liSNPs[srpair,] # y creas un genlight con ese par
    ...
    nMissLoc # Número de datos faltantes, considerando la original y la réplica
    ...
    shareMissLoc <- length(intersect(s,r)) # elements common not sample and replicate
    shareMissLoc 
    ...
    x<-cbind(pair, nloci, nMissLoc, MissTotProp, shareMissLoc, loci.mismatches, unshareMissLoc, loci.error.rate, n.loci.woNA, allele.mismatches, allele.error.rate)
    repliDiff<-rbind(repliDiff, x)

  }
  Parameter<-rep(parametro,times=npairs)
  repliDiff<-cbind(Parameter, repliDiff)
